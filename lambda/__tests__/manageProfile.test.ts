...
import { CreateProfileRequest, UpdateProfileRequest, UserProfile } from '../../src/types';
...
describe('Profile Management Lambda Handler', () => {
    ...
    describe('GET /profile', () => {
        ...
        it('should return 401 when authorization header is missing', async () => {
            const event = createMockEvent('GET', '/profile', null, { Authorization: '' });
            const result = await handler(event);
    ...
    describe('PUT /profile', () => {
        ...
        it('should update user profile successfully', async () => {
            const updatedProfile = { ...mockUserProfile, firstName: 'Jane', displayName: 'Jane Doe' };
            ...
            expect(mockVerifyTokenAndGetUserId).toHaveBeenCalledWith(validAuthHeader);
            expect(mockUpdateLastLogin).toHaveBeenCalledWith(mockUserId);
            expect(mockUpdateUserProfile).toHaveBeenCalledWith(mockUserId, updateRequest);
        });
    ...
    describe('PUT /profile/onboarding/complete', () => {
        it('should complete onboarding successfully', async () => {
            ...
            const responseBody = JSON.parse(result.body);
            expect(responseBody.success).toBe(true);
            expect(responseBody.data.message).toBe('Onboarding completed successfully');
            expect(responseBody.timestamp).toBeDefined();
            expect(responseBody.requestId).toBeDefined();
