...
export async function handler(
    ...
    try {
        ...
        const httpMethod = (event.requestContext as any)?.http?.method || event.requestContext?.httpMethod || event.httpMethod;
        const rawPath = (event as any).rawPath || event.requestContext?.path || event.path || '';
        ...
        } else if (httpMethod === 'PUT' && rawPath === '/profile/onboarding/complete') {
        ...
        } else if (error instanceof AuthError || (error as any)?.name === 'AuthError') {
            return createErrorResponse(401, (error as Error).message, requestId);
        } else if (error instanceof DatabaseError || (error as any)?.name === 'DatabaseError') {
...
async function handleGetProfile(
    ...
    if (!profile) {
        Logger.warn('Profile not found for authenticated user', { userId, requestId });
...
async function handleCreateProfile(
    ...
    try {
        ...
        return createErrorResponse(400, 'Invalid or missing request body', requestId);
    }
    ...
    let email: string;
    try {
        const accessToken = (event.headers.authorization || event.headers.Authorization || '').substring(7);
    ...
    Logger.info('Profile created successfully', { userId, username: createdProfile.username, requestId });
...
async function handleUpdateProfile(
    ...
    const forbiddenFields = ['userId', 'email', 'onboardingCompleted', 'createdAt', 'lastLoginAt'];
