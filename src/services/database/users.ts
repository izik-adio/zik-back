...
export async function isUsernameUnique(
  ...
  try {
    ...
    if (!isUnique && excludeUserId && result.Items) {
      ...
      isUnique =
        result.Items.length === 1 && result.Items[0].userId === excludeUserId;
    }
...
export async function updateUserProfile(
  ...
  if (updateData.displayName !== undefined) {
    ...
    expressionAttributeValues[':displayName'] =
  ...
  if (updateData.avatarUrl !== undefined) {
    ...
    expressionAttributeValues[':avatarUrl'] =
      updateData.avatarUrl?.trim() || null;
  }
...
export async function deleteUserProfile(userId: string): Promise<void> {
  try {
    ...
    Logger.info('User profile permanently deleted', { userId });
  } catch (error) {
